...
char LeftBumperString[] = "$PRSO400,1\r\n";
char RightBumperString[] = "$PRSO401,1\r\n";
...
void setup() {
  ...
  //connection = ADB::addConnection("tcp:1137", true, adbEventHandler);
	connection = ADB::addConnection("tcp:1140", true, adbEventHandler);
...
void clear_bumper() {
  ...
  servo[0].write(70); //STOP! We hit something!
	delay(500);
	Serial.println("NEUTRAL");
  servo[0].write(90); //STOP! We hit something!
	Serial.println("SLEEPING 5 seconds");
	delay(5000);
	while(digitalRead(LEFTBUMP) == LOW | digitalRead(RIGHTBUMP) == LOW ) {
	Serial.println("BACKINGUP");
		servo[0].write(70); //Backup slowly
	}
	Serial.println("STOPPING");
  servo[0].write(90); //STOP! We cleared the obstical
  bumper_state = 0;
  init_bumpers();
}                      
...
void init_bumpers() {
	...
	//attachInterrupt(2, bumper, FALLING);
	//attachInterrupt(3, bumper, FALLING);
}

void check_bumpers(){
	if ( digitalRead(LEFTBUMP) == LOW) {
  	Serial.println("Holy crap! I ran into something!");
  	clear_bumper();
    
    connection->writeString(LeftBumperString);
    Serial.print(LeftBumperString);
  }
    ...
    connection->writeString(RightBumperString);
    Serial.print(RightBumperString);
  }
...
void check_killswitch(){
	if ( digitalRead(KILLPIN) == LOW) {
		servo[0].write(90); // STOP MOVING! WE ARE DEAD.
		while(digitalRead(KILLPIN) == LOW) {
			Serial.println("Ahhh! You killed me!");
			delay(500);
		}
	}
}
